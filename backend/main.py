...
class SubtitleRemover:
    def __init__(self, vd_path, gui_mode=False):
        ...
        self.show_processing_details = True
    ...
    def video_inpaint(self, tbar, model):
        ...
        if len(sub_list) == 0:
            raise Exception(tr['Main']['NoSubtitleDetected'].format(self.video_path))
        continuous_frame_no_list = sub_detector.find_continuous_ranges_with_same_mask(sub_list)
        if self.show_processing_details:
        ...
        while True:
            ...
            # 如果是区间开始，则找到尾巴
                ...
                mask_area_coordinates = []
                # 1. 获取当前批次的mask坐标全集
                # Frame ranges are inclusive; include the end frame as well.
                for mask_index in range(start_frame_index, current_frame_index + 1):
                ...
                if not mask_area_coordinates:
                    # A single-frame or filtered interval still needs to be copied.
                ...
                for batch in batch_generator(frames_need_inpaint, config.getSttnMaxLoadNum()):
                    ...
                    if len(batch) >= 1:
                        ...
                        if len(inpainted_frames) != len(batch):
                            tbar.write(
                                f"warning: inpaint returned {len(inpainted_frames)} frames for {len(batch)} input frames; preserving originals"
                            )
                        for i, original_frame in enumerate(batch):
                            inpainted_frame = (
                                inpainted_frames[i]
                                if i < len(inpainted_frames)
                                and inpainted_frames[i] is not None
                                else original_frame
                            )
                            self.video_writer.write(inpainted_frame)
    ...
    def log_model(self):
        ...
        self.append_output(tr['Main']['SubtitleDetectionModel'].format(f"{config.subtitleDetectMode.value.value} ({providers})"))

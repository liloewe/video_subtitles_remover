...
# Keep the same geometric false-positive filters as v6_middle.py.
...
# Only white, high-contrast subtitle text is eligible for inpainting.
OCR_FILTER_WHITE_TEXT = setting("OCR_FILTER_WHITE_TEXT", "1").lower() in {
    "1", "true", "yes", "on"
}
...
def compute_subtitle_area(width: int, height: int):
    """Return the default subtitle area as (y_min, y_max, x_min, x_max)."""
    if height > width:
...
class PPOCRV6Detector:
    ...
    def _white_text_score(image, polygon):
        """Return bright-neutral and contrasting-bright pixel ratios for a box."""
        try:
            crop = PPOCRV6Detector._crop_polygon(image, polygon)
            if crop.size == 0 or crop.shape[0] < 2 or crop.shape[1] < 2:
                return 0.0, 0.0
            hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
            white = (hsv[:, :, 2] >= OCR_WHITE_MIN_VALUE) & (
                hsv[:, :, 1] <= OCR_WHITE_MAX_SATURATION
            )
            dark = hsv[:, :, 2] <= max(0, OCR_WHITE_MIN_VALUE - 35)
            dark_nearby = cv2.dilate(
                dark.astype(np.uint8), np.ones((3, 3), np.uint8)
            ).astype(bool)
    ...
    def __call__(self, image, *args, **kwargs):
        ...
        if results:
            all_boxes = np.asarray(results[0]["dt_polys"], dtype=np.float32)
        else:
            all_boxes = np.empty((0, 4, 2), dtype=np.float32)
        if all_boxes.size == 0:
            all_boxes = np.empty((0, 4, 2), dtype=np.float32)
        detected_polygons = all_boxes.astype(int).tolist()
        self.last_ocr_results = []
        self.last_ocr_error = None
        ...
        if OCR_REMOVE_MIN_Y_RATIO > 0:
            skipped_indexes.update(index for index, box in enumerate(all_boxes) if self._center_y(box) < min_center_y)
        ...
        for index, box in enumerate(all_boxes):
            ...
            if (box_height / max(1.0, frame_height) > OCR_MAX_BOX_HEIGHT_RATIO or box_width * box_height / frame_area > OCR_MAX_BOX_AREA_RATIO):
                skipped_indexes.add(index)
        kept_indexes = [index for index in range(len(all_boxes)) if index not in skipped_indexes]

        # First confirm that the detection is readable text.
        if self.recognizer is not None and kept_indexes:
            ...
            if should_recognize:
                ...
                self.last_ocr_results = self.recognize(image, all_boxes)
        ...
        # Then inspect only recognised text boxes for white subtitle pixels.
        white_scores = {}
        non_white_indexes = []
        if OCR_FILTER_WHITE_TEXT and kept_indexes:
            for index in kept_indexes:
                white_ratio, contrast_ratio = self._white_text_score(
                    image, all_boxes[index]
                )
                white_scores[index] = {
                ...
                if (
                    white_ratio < OCR_WHITE_MIN_RATIO
                    or contrast_ratio < OCR_WHITE_MIN_CONTRAST_RATIO
                ):
                    non_white_indexes.append(index)
            kept_indexes = [
                index for index in kept_indexes if index not in non_white_indexes
            ]
...
def process_video_direct(input_path, output_path):
    ...
    try:
        ...
        try:
            ...
            # SubtitleRemover treats an empty detection result as an error.
            # For an API job, no subtitles is a valid outcome: keep the source
            # video and continue publishing it instead of failing the request.
            if "No subtitles detected" not in str(exc):
                raise
            shutil.copyfile(input_path, output_path)
            log(f"未检测到字幕，保留原视频: {input_path}")
            return "no_subtitles"
    finally:
...
def process_job(job_id, video_url, callback_url):
    ...
    try:
        ...
        processing_status = process_video_direct(input_path, staging_path)
        ...
        if processing_status == "no_subtitles":
            payload["message"] = "未检测到字幕，已保留原视频"
...
if __name__ == "__main__":
    ...
    DOWNLOAD_DIR.mkdir(parents=True, exist_ok=True)
    PROCESSED_DIR.mkdir(parents=True, exist_ok=True)
    app.run(host="0.0.0.0", port=int(setting("PORT", "8027")))
